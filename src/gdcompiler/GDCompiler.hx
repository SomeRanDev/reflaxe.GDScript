...
class GDCompiler extends reflaxe.DirectToStringCompiler {
	...
	**/
	public override function onCompileEnd() {
		if(Context.defined("generate_godot_plugin")) {
			generatePlugin();
		}
	}
...
name="Reflaxe/GDScript Output"
description="Generated by Reflaxe/GDScript"
author=""
version=""
...
func _enter_tree():
	add_autoload_singleton(AUTOLOAD_NAME, "HxStaticVars.gd")
	${
		nodeClasses.map(function(cls) {
	...
	${
		resourceClasses.map(function(cls) {
...
func _exit_tree():
	...
	${nodeClasses.map((cls) -> 'remove_custom_type("${cls.name}")').join("\n")}
	${resourceClasses.map((cls) -> 'remove_custom_type("${cls.name}")').join("\n")}
');
	}
