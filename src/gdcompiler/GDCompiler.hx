...
class GDCompiler extends reflaxe.DirectToStringCompiler {
	...
	**/
	...
	/**
		Set to `true` when compiling starting field assignments in a constructor.
		These are automatically generated by the Haxe compiler from default field values.
		We want to undo this behavior and eliminate these assignments from the constructor.
	**/
	var compilingConstructorDefaultAssignments: Bool = false;
...
';
	...
	**/
				...
				if(!isSignal) {
					...
					var gdScriptVal = if(f.expr != null) {
						...
						if(isConstructor) {
							...
							compilingConstructorDefaultAssignments = true;
						}
						...
						if(isConstructor) {
							...
							compilingConstructorDefaultAssignments = false;
						}
	...
	public function compileExpressionImpl(expr: TypedExpr, isTopLevel: Bool): Null<String> {
		// See `compilingConstructorDefaultAssignments` doc comment.
		if(compilingConstructorDefaultAssignments) {
			switch(expr.expr) {
				case TBinop(OpAssign, { expr: TField({ expr: TConst(TThis) }, FInstance(_, _, _)) }, e2): {
					return null;
				}
				case _: {
					compilingConstructorDefaultAssignments = false;
